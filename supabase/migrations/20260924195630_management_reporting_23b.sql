-- TASK-23B: a controlled catalogue and a single guarded, read-only snapshot.
create table public.reporting_measure_definitions_23b (
 ...
 source_read_interface text not null,
 included_states text[] not null,
 excluded_states text[] not null,
 unit text not null,
 ...
 publication_actor text not null,
 published_at timestamptz not null default transaction_timestamp(),
 primary key (measure_code, definition_version)
);
alter table public.reporting_measure_definitions_23b enable row level security;
revoke all on public.reporting_measure_definitions_23b from public, anon, authenticated;

create function private.reporting_definition_immutable_23b() returns trigger
language plpgsql set search_path = '' as $$
begin
 raise exception 'Published reporting definitions are immutable';
end $$;
create trigger reporting_definition_immutable_23b before update or delete
on public.reporting_measure_definitions_23b for each row
execute function private.reporting_definition_immutable_23b();

insert into public.reporting_measure_definitions_23b
(measure_code,definition_version,display_name,source_domain,source_owner,source_grain,source_read_interface,
 included_states,excluded_states,unit,numerator,denominator,date_rule,timezone,cancellation_rule,
 correction_rule,coverage_rule,publication_actor)
select code,1,label,domain,domain,grain,'public.management_report_23b',inc,exc,unit,numerator,denominator,
 date_rule,'Europe/London',cancel_rule,'Current source revision only; history unavailable in v1',coverage,
 'TASK-23B approved source migration'
from (values
 ('CLIENTS','Clients','CRM','crm_organisations.id',array['CLIENT']::text[],array['PROSPECT','FORMER_CLIENT','PARTNER']::text[],'records','count(distinct id)',null::text,'Current state at query time','Current state only','CRM guarded source population'),
 ('SITES','Sites','Sites','sites.id',array['DRAFT','ACTIVE','RETIRED']::text[],array[]::text[],'records','count(distinct id)',null,'Current state at query time','Current state only','Sites guarded source population'),
 ('SITE_SERVICES','Site Services','Site Service','site_services.id',array['DRAFT','ACTIVE','PAUSED','ENDED']::text[],array[]::text[],'records','count(distinct id)',null,'Current state at query time','Current state only','Site Service guarded source population'),
 ('EVENTS','Events','Event','operational_events.id',array['PLANNING','CONFIRMED','LIVE','COMPLETED','CANCELLED']::text[],array[]::text[],'records','count(distinct id)',null,'Current state at query time','Current state only','Event guarded source population'),
 ('REQUIRED','Required positions','Event and Site Shift','typed requirement/demand UUID',array['PLANNED']::text[],array['CANCELLED']::text[],'positions','sum(required_quantity)',null,'Source London service_date in [start,end)','Cancelled demand excluded from current','08D static materialisation separately disclosed'),
 ('ALLOCATED','Allocated positions','Event and Site Shift','typed active allocation UUID',array['ALLOCATED','ACCEPTED']::text[],array['DECLINED','CANCELLED']::text[],'positions','count(active allocation id)',null,'Parent source London service_date in [start,end)','Cancelled allocations excluded from current','08D static materialisation separately disclosed'),
 ('ACCEPTED','Accepted positions','Event and Site Shift','typed active allocation UUID',array['ACCEPTED']::text[],array['ALLOCATED','DECLINED','CANCELLED']::text[],'positions','count(ACCEPTED allocation id)',null,'Parent source London service_date in [start,end)','Cancelled allocations excluded from current','08D static materialisation separately disclosed'),
 ('REMAINING','Remaining positions','Event and Site Shift','typed requirement/demand UUID',array['PLANNED']::text[],array['CANCELLED']::text[],'positions','sum(greatest(required-allocated,0))',null,'Source London service_date in [start,end)','Cancelled demand excluded from current','08D static materialisation separately disclosed'),
 ('EXPLICIT_CONFLICTS','Explicit availability conflicts','Availability','typed active allocation UUID',array['UNAVAILABLE_CONFLICT','COVERAGE_NO_LONGER_DECLARED']::text[],array['NOT_DECLARED','NOT_FULLY_COVERED']::text[],'allocations','count(explicit conflict allocation id)',null,'Parent source London service_date in [start,end)','Cancelled allocations excluded','Only 07A/07B indicators'),
 ('MISSING_DECLARATION','Missing declaration','Availability','typed active allocation UUID',array['NOT_DECLARED']::text[],array[]::text[],'allocations','count(NOT_DECLARED allocation id)',null,'Parent source London service_date in [start,end)','Cancelled allocations excluded','Only 07A/07B assessment'),
 ('PARTIAL_COVERAGE','Partial coverage','Availability','typed active allocation UUID',array['NOT_FULLY_COVERED']::text[],array[]::text[],'allocations','count(NOT_FULLY_COVERED allocation id)',null,'Parent source London service_date in [start,end)','Cancelled allocations excluded','Only 07A/07B assessment'),
