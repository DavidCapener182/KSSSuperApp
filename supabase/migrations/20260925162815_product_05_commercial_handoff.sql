-- PRODUCT-05: factual Office commercial context. Read only; source records retain authority.
create function public.commercial_handoff(p_organisation uuid, p_opportunity uuid default null)
    ...
    'mobilisations', (select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'sources', (select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'name', case when not private.mobilisation_link_valid(m.id,l.source_type,l.source_id) then null when l.source_type = 'SITE' then (select s.name from public.sites s where s.id = l.source_id)
          when 'SITE_SERVICE' then (select s.name from public.site_services s where s.id = l.source_id)
...
create function public.commercial_mobilisation_portfolio(p_offset integer default 0, p_limit integer default 25)
returns jsonb language plpgsql stable security definer set search_path = '' as $$
begin
  if not private.crm_authorised() or p_offset < 0 or p_offset > 10000 or p_limit < 1 or p_limit > 50
    then raise exception 'Mobilisation portfolio read denied'; end if;
  return jsonb_build_object('total',(select count(*) from public.mobilisations),
    'items',(select coalesce(jsonb_agg(to_jsonb(x) order by x.created_at desc,x.id desc),'[]'::jsonb)
      from (select m.id,m.title,m.status,m.organisation_id,o.name organisation_name,
        owner.display_name owner_name,m.target_go_live,m.created_at,
        (select count(*) from public.mobilisation_actions a where a.mobilisation_id=m.id and a.state not in ('DONE','CANCELLED')) open_actions,
        (select count(*) from public.mobilisation_blockers b where b.mobilisation_id=m.id and b.resolved_at is null) open_blockers,
        (select a.title from public.mobilisation_actions a where a.mobilisation_id=m.id and a.state not in ('DONE','CANCELLED')
          order by a.due_on nulls last,a.template_ordinal nulls last,a.created_at,a.id limit 1) next_work
      from public.mobilisations m join public.crm_organisations o on o.id=m.organisation_id
      join public.people owner on owner.id=m.owner_person_id
      order by m.created_at desc,m.id desc limit p_limit offset p_offset) x));
end $$;
revoke all on function public.commercial_mobilisation_portfolio(integer,integer) from public,anon,authenticated;
grant execute on function public.commercial_mobilisation_portfolio(integer,integer) to authenticated;
