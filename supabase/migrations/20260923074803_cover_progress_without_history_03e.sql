-- TASK-03E: accurate cover progress without granting historical evidence reads.
...
declare c public.onboarding_cases%rowtype; req record; completed integer:=0; completed_ids uuid[]:=array[]::uuid[];
...
begin
  ...
  for req in select cr.id,cr.document_request_id,d.code from public.onboarding_case_requirements cr
    ...
    if done then completed:=completed+1; completed_ids:=array_append(completed_ids,req.id); end if;
  end loop;
  return jsonb_build_object('verifiedCount',completed,'totalCount',6,'completedRequirementIds',completed_ids,
...
begin
  ...
  select jsonb_build_object('caseId',c.id,'teamId',c.team_id,
    ...
    'completedRequirementIds',private.onboarding_case_triage(c.id)->'completedRequirementIds',
