-- TASK-07B: read-only, exact-source workforce projections. No rota table or write path.
create index if not exists staffing_service_week_07b_idx
 on public.event_staffing_requirements(service_date,report_at,event_id,id) where state='PLANNED';

create function public.workforce_week(p_week date,p_event uuid default null,p_site uuid default null,
...
begin
 ...
 with scoped as materialized (
  ...
  where r.state='PLANNED' and e.status in ('PLANNING','CONFIRMED','LIVE')
   ...
   and (nullif(trim(p_client),'') is null or o.name ilike '%'||trim(p_client)||'%')
...
revoke all on function public.workforce_week(date,uuid,uuid,text,uuid,uuid,boolean,boolean,integer,integer) from public,anon,authenticated;
grant execute on function public.workforce_week(date,uuid,uuid,text,uuid,uuid,boolean,boolean,integer,integer) to authenticated;

create function public.workforce_staff_choices(p_search text default '',p_offset integer default 0,p_limit integer default 20)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare result jsonb;
begin
 if not private.operational_authorised() or length(coalesce(p_search,''))>80 or coalesce(p_search,'') ~ '[[:cntrl:]]'
  or p_offset is null or p_offset<0 or p_offset>10000 or p_limit is null or p_limit<1 or p_limit>30
  then raise exception 'Workforce Staff search denied'; end if;
 with staff as materialized (
  select p.id,p.display_name from public.people p where exists (
   select 1 from public.role_assignments ra where ra.person_id=p.id and ra.role_code='SECURITY_STAFF'
    and ra.revoked_at is null and ra.effective_from<=now() and (ra.effective_until is null or ra.effective_until>now()))
   and (nullif(trim(p_search),'') is null or p.display_name ilike '%'||trim(p_search)||'%')
 ), page as (select * from staff order by display_name,id offset p_offset limit p_limit)
 select jsonb_build_object('total',(select count(*) from staff),
  'items',coalesce((select jsonb_agg(jsonb_build_object('id',id,'display_name',display_name)
   order by display_name,id) from page),'[]'::jsonb)) into result;
 return result;
end $$;
revoke all on function public.workforce_staff_choices(text,integer,integer) from public,anon,authenticated;
grant execute on function public.workforce_staff_choices(text,integer,integer) to authenticated;

create function public.workforce_person_week(p_person uuid,p_week date,p_offset integer default 0,p_limit integer default 30)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare result jsonb;
begin
 if not private.operational_authorised() or p_week is null or extract(isodow from p_week)<>1
 ...
 with scoped as materialized (
  select a.id,a.status,r.event_id,r.id as requirement_id,r.service_date,r.report_at,r.shift_starts_at,r.shift_ends_at,
   r.area_label,rd.display_name as role_name,e.name as event_name,s.name as site_name,
   private.availability_assessment_07a(a.person_id,r.report_at,r.shift_ends_at) as availability,
 ...
 ), page as (select * from scoped order by service_date,report_at,id offset p_offset limit p_limit)
 select jsonb_build_object('total',(select count(*) from scoped),
  'items',coalesce((select jsonb_agg(to_jsonb(page) order by service_date,report_at,id) from page),'[]'::jsonb)) into result;
 return result;
end $$;
revoke all on function public.workforce_person_week(uuid,date,integer,integer) from public,anon,authenticated;
grant execute on function public.workforce_person_week(uuid,date,integer,integer) to authenticated;

create function public.my_schedule(p_week date,p_offset integer default 0,p_limit integer default 50)
returns jsonb language plpgsql stable security definer set search_path='' as $$
...
begin
 ...
 with work as materialized (
  select a.id,a.status,r.service_date,r.report_at,r.shift_starts_at,r.shift_ends_at,r.area_label,
   rd.display_name as role_name,e.name as event_name,s.name as site_name,s.reporting_point,
 ...
 select jsonb_build_object('week_start',p_week,'work_total',(select count(*) from work),
  'work',coalesce((select jsonb_agg(to_jsonb(w) order by w.service_date,w.report_at,w.id)
   from (select * from work order by service_date,report_at,id offset p_offset limit p_limit) w),'[]'::jsonb),
  'availability',coalesce((select jsonb_agg(to_jsonb(d) order by d.starts_at,d.id) from declarations d),'[]'::jsonb)) into result;
 return result;
end $$;
revoke all on function public.my_schedule(date,integer,integer) from public,anon,authenticated;
grant execute on function public.my_schedule(date,integer,integer) to authenticated;
