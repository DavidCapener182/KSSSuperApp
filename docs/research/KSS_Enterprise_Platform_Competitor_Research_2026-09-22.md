# KSS Enterprise Platform — Module-by-Module Product Research

**Research date:** 22 September 2026  
**Version:** 1.0 — research findings and proposed improvements  
**Scope:** All 17 original master sections, expanded into 30 separately assessed capability areas.  
**Companion:** `KSS_Enterprise_Platform_Master_Codex_Prompt_v2.md`

## Overall recommendation

Build the KSS-specific operational experience and common records. Borrow useful workflow patterns from specialist products, but integrate established training, payroll, accounting and other engines where that is the safer, more maintainable choice. A single employee/client experience does not require KSS to rebuild every system behind it.

The most useful additions are explicit verification and approval states; service-desk handling of questions and client changes; effective-dated financial snapshots; version-specific instructions; fresh inspection evidence; employee offboarding; accountable equipment custody; and visible synchronisation failures. These are design recommendations, not claims that the referenced vendors collectively provide a ready-made KSS solution.

## Method and limits

This is a desk-based review of official vendor product pages, help documentation and primary technical guidance. It is not hands-on testing, a procurement tender, an exhaustive market survey or an independently scored best-product ranking. The **Documented pattern** paragraph records the sourced observation; **Proposed KSS requirements**, **Acceptance test**, **Delivery recommendation** and **Avoid** are our design judgement for this project.

No prices, performance improvements, security certification, UK legal sufficiency or compatibility with existing KSS systems have been inferred. Help pages have different revision dates. A documented feature can require a particular subscription, deployment model or configuration. Existing Footasylum Audits, MagSecure and the unnamed training platform were not tested or inspected internally in this review. Confirm their actual interfaces before committing to integration. No subscriptions were purchased and no production application was built or deployed.

User-requested baseline features remain in the companion master. Research-led additions are marked as proposals so brainstorming does not silently become authorisation for a larger first release. Recommendations concerning employment checks, finance, monitoring or emergency response need the appropriate KSS policy, professional review and operational sign-off before use.

## Coverage of the existing master
...
Each R-number identifies a capability, not necessarily a separate menu or codebase. Reuse shared records and services. The acceptance test is the minimum useful proof for that area; turn it into detailed test cases when its build scope is approved. The next-action sentence is an actionable discovery or implementation backlog item.

---

# R01. Platform structure and shared records
**Documented pattern:** TrackTik connects security delivery with back-office work. HubSpot documents explicit associations between different record types. These are useful references for a connected business model, rather than a dashboard hiding separate databases. [S01] [S14]

**Proposed KSS requirements**
...
**Acceptance test:** Create one client with two sites and one event. Its contacts, documents and work are visible in the appropriate views using the same IDs; a contact edit appears consistently and cannot leak to an unrelated client.

**Delivery recommendation:** Foundation: build the KSS domain model; integrate specialist engines. Preserve the existing applications until replacement workflows are proven.

**Avoid:** Copying the appearance of an ERP while leaving every module with its own employee or client table; importing vendor product claims as measured KSS benefits.

**Next action:** Inventory existing records and decide the authoritative system for each entity and field before designing imports.

---

# R02. Roles, administration and sensitive access
**Documented pattern:** Microsoft Entra PIM provides time-limited, approval-controlled privileged access. OWASP ASVS supplies a framework for verifying application security controls. The useful pattern is controlled privilege, not an unrestricted administrator checkbox. [S04] [S37]
...
**Acceptance test:** A supervisor assigned to Site A is denied Site B through the UI, API, exported report and document URL. A temporary finance grant expires and subsequent requests fail. The audit history identifies the granting user and reason.

**Delivery recommendation:** Foundation: build resource-level authorisation; integrate identity/MFA with a suitable provider. Evaluate advanced time-limited privileges in the security design.

**Avoid:** A single broad Office/Admin role becoming a route to every private staff record; assuming identity-provider login handles application-level client isolation.

**Next action:** Produce a permission matrix using actual KSS job responsibilities and name the authorised approvers for privilege changes.

---

# R03. Navigation, dashboards and the daily work area
**Documented pattern:** TrackTik documents operational activity and live attendance views. HubSpot uses filtered record activity timelines. Together they suggest a homepage for immediate action and record pages for context, rather than forcing users through a large menu. [S03] [S15]
...
**Acceptance test:** A guard reaches their next shift, correct current site briefing and expense form without accessing office navigation. A manager opens a staffing-gap number and sees exactly the shifts contributing to it.

**Delivery recommendation:** Foundation: build a shared navigation shell and role-specific work queues before adding decorative dashboards.

**Avoid:** An attractive homepage with unactionable totals; separate navigation rules that disagree with server permissions; endless confirmation pop-ups obscuring urgent tasks.

**Next action:** Prototype guard, office, supervisor and client journeys with realistic records and agree the first five actions each role needs.

---

# R04. File architecture, controlled documents and migration
**Documented pattern:** SharePoint already supports approval, version history and draft/published controls. The opportunity is to improve the current folder-led workflow, not assume SharePoint lacks document-management capabilities. [S05] [S06]
...
**Acceptance test:** Publish SOP version 2 after version 1 is acknowledged. Staff see version 2 as current and version 1 acknowledgement remains historically correct. An HR attachment linked to a site stays private.

**Delivery recommendation:** Foundation: build metadata, context and permissions. Decide retain/integrate/migrate storage after inspecting the current configuration.

**Avoid:** An unnecessary wholesale SharePoint replacement; copied certificates treated as new inspection observations; editing a live document while claiming the prior approval still applies.

**Next action:** Choose pilot document types and agree ownership, approval audiences, retention decisions and the source-of-truth boundary.

---

# R05. Recruitment, onboarding and the joiner–mover–leaver lifecycle
**Documented pattern:** BambooHR documents reusable onboarding packets, electronic signatures and offboarding workflows. These suggest extending the proposed onboarding checklist into an employee lifecycle with accountable hand-offs. [S07]
...
**Acceptance test:** An applicant completes all uploads but remains non-deployable until required reviews are verified. A leaver loses access while their approved past hours and evidence remain available to authorised staff.

**Delivery recommendation:** Foundation: build lifecycle and requirement orchestration; integrate identity checks or signing where justified. No automated employment decisions.

**Avoid:** Importing US onboarding checks into a UK workflow; blanket deletion of leavers; treating 100% checklist completion as legal compliance.

**Next action:** Confirm actual KSS onboarding documents, verification responsibilities, signing needs and leaver/rehire process.

---

# R06. Training integration and competency management
**Documented pattern:** Moodle Workplace documents rule-driven learning assignments and recurring compliance training. The transferable pattern is assigning training from role and requirement changes, then retaining dated completion evidence. [S08] [S09]
...
**Acceptance test:** A supervisor qualification expiring before a proposed shift ends produces the defined blocker. A repeated completion callback creates one result. Removing an assignment does not erase the historic certificate.

**Delivery recommendation:** Foundation: build requirement/status views; integrate the LMS. Defer building a course authoring and assessment engine.

**Avoid:** Promising an embedded course player before confirming provider support; allowing an administrator to waive a mandatory legal requirement by adding a note.

**Next action:** Run one test user through assignment, launch, completion, expiry and reconciliation using the actual existing LMS.

---

# R07. Employee portal, announcements and leave
**Documented pattern:** Connecteam documents targeted updates with acknowledgements and employee time-off requests with approval states. These are useful references for making the staff profile a daily self-service workspace. [S10] [S11]
...
**Acceptance test:** An employee submits leave affecting a booked shift. The responsible planner receives an actionable conflict and the employee can track the decision. A targeted announcement is not shown to unrelated staff.

**Delivery recommendation:** Foundation for profile and self-service; operational release for leave/absence integration. Build the interface around core records.

**Avoid:** An employee portal that is just a document directory; coercive pop-ups blocking reporting; broad publication of directory or private HR information.

**Next action:** Agree staff-facing tabs and which updates staff can make directly versus request for review.

---

# R08. Staff questions and internal service desk
**Documented pattern:** Jira Service Management separates user-facing request categories from internal workflows and brings requests from several channels into managed queues. This is a stronger reference for staff questions than unstructured chat alone. [S12] [S13]
...
**Acceptance test:** A pay query links to the employee’s shift and approved hours. A finance internal note is absent from the staff view and email notification. Reopening the case preserves its original history.

**Delivery recommendation:** Foundation: build a small service-desk workflow using shared requests, tasks and permissions; do not build a Slack replacement.

**Avoid:** Questions vanishing into a message feed; confidential replies exposed by forwarding; turning every message into a new duplicate task.

**Next action:** Define request categories, receiving teams, public/private fields and realistic response targets.

---

# R09. CRM, contacts, opportunities and account history
**Documented pattern:** HubSpot documents associated contacts, organisations and deals alongside record-level activity timelines. Its relevant strength is relationship context: different records remain connected without becoming copies of one another. [S14] [S15]
...
**Acceptance test:** A contact linked to two authorised sites remains a single record. Converting an opportunity retains quote history and opens the right mobilisation template without re-entering names and addresses.

**Delivery recommendation:** Foundation: build lightweight shared CRM; integrate messaging selectively. Defer extensive marketing automation.

**Avoid:** A sales pipeline isolated from delivery; ambiguous primary-contact fields; counting open opportunity values as secured revenue.

**Next action:** Agree relationship labels, conversion rules and a duplicate-merge process before importing contact lists.

---

# R10. Client requests, approvals and contract changes
**Documented pattern:** Jira Service Management provides trackable requests; HubSpot provides commercial record relationships. Combining these patterns makes a client change a managed service and commercial event rather than an email someone must remember. [S12] [S13] [S14]
...
**Acceptance test:** A client requests two additional posts. Approval updates the staffing requirement and chargeable work exactly once, while the original contract and its earlier values remain reproducible.

**Delivery recommendation:** Operational release: build on the shared Request and Approval models. Integrate communication channels only after permissions are proven.

**Avoid:** Delivering extra work without a traceable change record; presenting a sent quote as accepted; exposing internal cost calculations to clients.

**Next action:** Define which variations require client sign-off, a purchase order or senior commercial approval.

---

# R11. Site records and mobilisation readiness
**Documented pattern:** TrackTik’s post orders are scoped to operational contexts. Asana’s templates can use role-based responsibilities and relative dates. These suggest a site workspace with a repeatable mobilisation process rather than a folder and a start date. [S02] [S16]
...
**Acceptance test:** Moving a proposed go-live date recalculates only eligible incomplete tasks. Completed work retains its historical dates. A missing required site induction remains visible as a deployment blocker.

**Delivery recommendation:** Foundation: site records and basic mobilisation templates. Operational release: fuller live-readiness controls.

**Avoid:** Rebuilding a mobilisation checklist for every site; fixed due dates going stale; copying old task completion into a new deployment.

**Next action:** Choose one static site and one event mobilisation as templates and identify genuine stop/go gates.

---

# R12. Events, accreditation and logistics
**Documented pattern:** Eventree documents advancing and accreditation workflows, including QR passes. Blerter presents event operations with runsheets, locations and task/incident functions. These are relevant references for preparing and delivering an event, not just allocating shifts. [S18] [S19]
...
**Acceptance test:** A confirmed member of staff whose zone authorisation is missing is shown as deployed but not access-ready. Reusing an event template creates new operational records and no historic attendance.

**Delivery recommendation:** Operational release: build event records, versioned deployment and readiness. Integrate accreditation; defer public ticketing and unnecessary event-sales functionality.

**Avoid:** Equating SIA validity with site/zone authorisation; creating a full public ticketing platform unrelated to KSS delivery.

**Next action:** Map one festival’s organiser accreditation hand-off and one football fixture’s briefing, attendance and post-event approval chain.

---

# R13. Rostering, suitability and deployment decisions
**Documented pattern:** Deputy describes scheduling that considers availability, training and configurable planning constraints. Moodle Workplace supplies a useful model for requirement-driven training records. The KSS opportunity is explainable suitability, not just filling names into posts. [S20] [S08]
...
**Acceptance test:** Two overlapping assignments are flagged. A qualification valid today but expired during next week’s shift blocks that proposed assignment under the configured requirement. The planner can see the reason without viewing private screening documents.

**Delivery recommendation:** Operational release: integrate existing rostering initially where practical; build KSS eligibility and exception views. Defer unsupervised AI allocation.

**Avoid:** Automated preference scores being mistaken for objective staff quality; exposing private HR material as the reason for rejection.

**Next action:** Document mandatory qualifications and client/site requirements, then test actual rostering export/API capability.

---

# R14. Attendance, timesheets and pay/charge rules
**Documented pattern:** PARiM documents distinct pay and charge rules with person, role, employment-type, location and client conditions, and cautions about effective date ranges. Xero’s UK API distinguishes stages in timesheet processing. These are useful references for a traceable financial hand-off. [S21] [S30]
...
**Acceptance test:** Approve a night shift, close its pay/invoice period, then change next month’s rate. Historic amounts remain unchanged and reproducible. A missing charge rule blocks invoice preparation instead of generating a zero-valued line.

**Delivery recommendation:** Operational release: integrate attendance where supported, build approval and financial evidence controls. Payroll processing remains with the selected payroll system.

**Avoid:** Checking in being treated as client-approved billable work; retroactive rate changes silently altering closed history; copying obsolete wage examples from vendor help.

**Next action:** Create test examples for KSS pay/charge combinations, overnight shifts, breaks and disputed hours, with Finance-approved expected results.

---

# R15. Tasks, projects and reusable work templates
**Documented pattern:** Asana documents reusable project templates, relative due dates and dependencies. The useful elements for KSS are repeatable work and visible blockers; these complement the requested Trello-style board interface. [S16] [S17]
...
**Acceptance test:** A fixture date changes. Its outstanding staffing and briefing deadlines move appropriately, while completed tasks and post-match approval history remain intact. The same task appears consistently in My Work and the event.

**Delivery recommendation:** Foundation: build a focused task system; keep Trello as the development backlog until replacement use is deliberate.

**Avoid:** Trying to reproduce every project-management feature; making all finance tasks due on event day; treating board-column movement as proof that work occurred.

**Next action:** Agree task states and turn one recurring operational workflow into a tested relative-date template.

---

# R16. Patrols, checkpoints and site instructions
**Documented pattern:** TrackTik documents QR/NFC checkpoint patrols and site/post-specific orders. The useful pattern is linking the checkpoint, required action and supporting operational instruction. [S02]
...
**Acceptance test:** A staff member opens a checkpoint by NFC, records a defect and submits once. The patrol observation and a linked follow-up task appear against the correct site. A replayed request does not create duplicate observations.

**Delivery recommendation:** Initial release: app hub and source mapping. Operational release: integrated status and follow-up; replace the patrol engine only after a demonstrated need.

**Avoid:** Claiming a tag scan proves a full patrol or prevents all spoofing; migrating an operational app before staff can complete and reconcile real patrols.

**Next action:** Test a real MagSecure checkpoint link, export format and staff authorisation flow, including failed connectivity.

---

# R17. Live incidents, command and handover
**Documented pattern:** D4H documents configurable incident status boards and operational task checklists. These suggest separating incoming reports, the incident’s common picture and the actions assigned to responders. [S22] [S23]
...
**Acceptance test:** Two reports about the same incident can be linked while retaining both originals. A handover records the accepting manager. A restricted safeguarding attachment is absent from the general status board and client export.

**Delivery recommendation:** Operational release: build case linkage and handover using shared records; evaluate specialist incident tooling for complex command needs.

**Avoid:** An editable free-text log with no correction history; claiming the app replaces radios, emergency services or trained incident command.

**Next action:** Define incident categories, escalation responsibilities, minimum information and the separation between internal and shareable records.

---

# R18. Lone-worker welfare and escalation
**Documented pattern:** TrackTik advertises lone-worker alert functions. SafetyCulture explicitly distinguishes offline-supported work from unsupported lone-worker and notification functions. The important lesson is that collecting data and delivering an emergency alert are not the same capability. [S02] [S36] [S41]
...
**Acceptance test:** In a supervised test, disable connectivity during an alert. The device states it has not been transmitted and provides the agreed fallback. A transmitted alert that nobody acknowledges is visibly escalated under the approved protocol.

**Delivery recommendation:** Safety-gated workstream: integrate or pilot only after operational and privacy assessment; never make the MVP the sole emergency channel.

**Avoid:** Treating a missed scan as automatic proof a guard is unsafe; false assurance from an offline panic button; unapproved employee surveillance.

**Next action:** Confirm whether KSS needs a welfare service, who monitors it and what approved response/fallback must happen out of hours.

---

# R19. Assets, keys, equipment kits and visitor access
**Documented pattern:** Snipe-IT documents asset check-out/check-in, acceptance, kits and maintenance history. TrackTik includes visitor management functions. These suggest extending the planned asset list into accountable custody, with access workflows only where needed. [S24] [S02]
...
**Acceptance test:** Issue a radio kit and return it with one damaged component. The kit is not shown as fully available and a maintenance action is opened. Key custody is traceable without exposing unrelated site access data.

**Delivery recommendation:** Operational release for equipment; optional later visitor/contractor workflow based on actual site need. Evaluate integration with existing asset/evidence systems.

**Avoid:** One spreadsheet row for a whole kit with no components; exposing security-sensitive keys or video to general office users.

**Next action:** Define KSS asset classes and pilot a kit issue/return at one event and one permanent site.

---

# R20. Audits, compliance evidence and corrective actions
**Documented pattern:** SafetyCulture documents preventing duplicated inspections and linking existing actions to inspections. Blerter documents hazard-led event workflows. The relevant pattern is reusable questions with fresh evidence and continuing corrective actions. [S25] [S26] [S42]
...
**Acceptance test:** Duplicate an audit template for a different store. No prior photographs or answers appear. A PAT evidence requirement opens the actual relevant certificate, and an unresolved action can be linked without duplication.

**Delivery recommendation:** Foundation: evidence classification and action records. Operational release: richer audit workflow; integrate the existing Footasylum audit application first.

**Avoid:** Pre-filling Yes as if a new check were completed; old images being passed off as current; software status being presented as certification of legal compliance.

**Next action:** Test an actual Footasylum-style audit with certificate, question-specific weekly-check evidence and a repeat corrective action.

---

# R21. Expenses, receipts and reimbursement approval
**Documented pattern:** Ramp documents conditional evidence requirements and expense approval flows, including separation-of-duties controls. These are useful process references; they are not a recommendation to adopt a particular banking or expense provider. [S27] [S28]
...
**Acceptance test:** A manager submits their own expense; it routes to another approver. A possible duplicate is flagged without an accusation. The employee sees approved but not paid until the payment reference is confirmed.

**Delivery recommendation:** Early self-service release: build capture and status. Operational/finance release: integrate reimbursement processing and reconciliation.

**Avoid:** Auto-paying an extracted receipt; an approval tick presented as a bank transfer; taking card or banking product availability for granted.

**Next action:** Agree categories, evidence requirements, approval thresholds, payment confirmation source and treatment of mileage.

---

# R22. Payroll, accounting and invoicing
**Documented pattern:** Xero’s UK ecosystem documents time-data integrations and payroll processing states; PARiM separates pay from client charges. The useful pattern is approved operational data feeding a specialist financial system, not duplicating the whole accounting engine. [S29] [S30] [S21]
...
**Acceptance test:** Export a batch, simulate partial failure, and retry. Only missing items are created; accepted records retain stable references. A payroll acknowledgement does not set employee reimbursement to paid.

**Delivery recommendation:** Integrate statutory payroll, accounting, tax and payments. Build KSS-specific operational preparation, approvals, exceptions and commercial visibility.

**Avoid:** A new in-house PAYE/tax/banking engine in the MVP; assuming Xero is KSS’s current provider; silent duplication after connector retries.

**Next action:** Confirm payroll/accounting provider, supported interfaces, finance mapping and a sandbox reconciliation test with Finance.

---

# R23. Client portal and controlled publication
**Documented pattern:** Jira Service Management offers requester-facing portals. Power BI documents role-related limits to row-level security. These are useful reminders that a client-facing view requires a deliberate publication and access boundary. [S12] [S31]
...
**Acceptance test:** A client user changes an invoice/report identifier and receives no other client’s data. A raw safeguarding incident remains hidden while its approved service summary can be published to the intended audience.

**Delivery recommendation:** Later operational release after internal data and publication rules are proven. Integrate external analytics cautiously.

**Avoid:** A client toggle that reveals raw incident tabs; giving clients administrative or contributor permissions in an analytics workspace.

**Next action:** Agree a publish/share matrix with separate rules for incident summaries, audits, financial records and operational documents.

---

# R24. Reporting, business intelligence and operational measures
**Documented pattern:** Power BI supports row-level security but documents that workspace Admin, Member and Contributor roles are not restricted by those rules in the same way as Viewers. The lesson is to test the complete reporting permission model. [S31]
...
**Acceptance test:** A manager opens an unfilled-hours metric and reconciles it to its shifts. The same dashboard for a client excludes other sites and private cost fields. Missing patrol data reads unknown, not fully compliant.

**Delivery recommendation:** Foundation for a few defined exception metrics; later analytics once source data reconciles.

**Avoid:** Charts before definitions; client-side filters as isolation; confident percentages calculated from incomplete integration data.

**Next action:** Agree the initial metric dictionary and validate each number against a small manually checked pilot dataset.

---

# R25. Knowledge base, trusted search and procedural guidance
**Documented pattern:** Notion documents owned, verified wiki content with review expiry. SharePoint provides controlled versions. The useful combination is discoverable knowledge with an accountable owner and an identifiable approved source. [S32] [S06]
...
**Acceptance test:** A superseded procedure no longer appears as the current answer. A person without HR access cannot discover a private article through search snippets, suggestions or AI citations.

**Delivery recommendation:** Foundation: structured guidance and search. Later: richer verification and learning from unanswered questions.

**Avoid:** A second folder of unofficial SOP copies; a permanent verified badge on an out-of-date procedure.

**Next action:** Choose content owners and map the most common staff questions to approved procedures with review dates.

---

# R26. Existing app hub and integration management
**Documented pattern:** TrackTik and Xero describe ecosystems that connect specialised systems. The transferable pattern is selective integration with defined responsibilities; a vendor having integrations does not establish compatibility with the existing KSS applications. [S01] [S29]
...
**Acceptance test:** A connector is unavailable. The dashboard shows its last confirmed state and a visible stale warning, while the approved alternative workflow remains accessible. Retrying a sync does not duplicate staff.

**Delivery recommendation:** Foundation for app hub and external-ID mapping; integrate progressively after capability tests.

**Avoid:** Buying an integration platform before knowing the actual API needs; assuming SSO automatically enforces target-app permissions or record scopes.

**Next action:** Complete a capability and ownership matrix for every named KSS system, including licence tiers and a sandbox test.

---

# R27. Shared approvals and workflow automation
**Documented pattern:** Power Automate documents sequential and other approval patterns together with retry and error-handling guidance. These suggest treating workflow execution as traceable business work, not invisible background magic. [S33] [S34]
...
**Acceptance test:** The same expense submission event is received twice and creates one approval request. Changing its amount after approval triggers the configured reapproval. A failed notification is visible and retryable without approving the expense.

**Delivery recommendation:** Foundation for typed shared approvals and simple triggers; operational release for a broader rule engine. Integrate automation tooling where maintainable.

**Avoid:** An undocumented chain of scripts; a workflow marked complete because an email was sent; using a task tick to certify a legal check.

**Next action:** Choose initial triggers and approval routes, then specify retries, timeouts, fallback ownership and audit events.

---

# R28. Permission-aware AI assistance
**Documented pattern:** Microsoft’s Copilot security documentation explains the importance of underlying access controls when AI surfaces organisational information. OWASP’s verification approach reinforces testing the system rather than assuming an AI prompt can enforce security. [S35] [S37]
...
**Acceptance test:** A malicious instruction embedded in an uploaded document cannot cause data export or permission changes. A supervisor’s AI answer contains no private payroll fields. An unsupported financial question produces a stated information gap.

**Delivery recommendation:** Later release after permissions, evidence quality and records are reliable. Build a narrow, audited assistant, not a parallel ungoverned workflow engine.

**Avoid:** Adding a chatbot to messy overshared data and calling it secure; allowing AI-created summaries to become the original incident record.

**Next action:** Create a test set of useful KSS questions, forbidden disclosures, stale-data cases and action-confirmation scenarios.

---

# R29. Mobile usability, offline work and synchronisation
**Documented pattern:** SafetyCulture documents supported offline mobile activities and explicitly unsupported functions such as notifications and lone-worker features. This is a strong reference for specifying offline behaviour per action rather than promising that the whole application works offline. [S36]
...
**Acceptance test:** A guard captures an observation and photo without signal. They see pending upload, not submitted. Reconnecting uploads once and preserves the original observation time. An unsent emergency action clearly remains unsent.

**Delivery recommendation:** Foundation for resilient forms and clear state; richer offline capture after a field pilot and storage/privacy decision.

**Avoid:** A blanket works-offline claim; silent data loss or duplicate invoices after retries; storing unrestricted HR records in browser caches.

**Next action:** Agree the smallest offline pilot and test it on actual iOS/Android devices under poor connectivity.

---

# R30. Engineering, Codex delivery, testing and production gates
**Documented pattern:** OWASP ASVS provides security verification requirements, Playwright supports separate test projects, GitHub documents deployment protection, and OpenAI’s Codex guidance emphasises task context and repository instructions. These are delivery references, not evidence that generated code is production-ready. [S37] [S38] [S39] [S40]
...
**Acceptance test:** A release cannot be presented as ready without recorded test outputs, permission checks, migration/rollback procedure and an approved pilot decision. A restore test demonstrates that evidence links and authorisation still work.

**Delivery recommendation:** Foundation across every phase. Tool choice and hosting remain to be confirmed from repository, skills, costs and data requirements.

**Avoid:** One giant Codex instruction followed by an untested deployment; fake test results; production credentials or real sensitive staff files in prompts and fixtures.

**Next action:** Create the repository delivery standard and choose one end-to-end pilot slice before estimating the rest of the build.

---

# Consolidated scope recommendations

Keep in the core planning scope: shared records, robust access control, the app hub, documents, task ownership, onboarding, training status and useful employee self-service. Implement one end-to-end slice before spreading effort across every dashboard.

Promote for explicit consideration: leave/absence, offboarding, request/case queues, contract variations, handovers, evidence-linked corrective actions, kit custody and integration-health visibility. Several are extensions of existing modules rather than entirely new applications.

Assess separately before reliance: welfare/SOS escalation, sensitive monitoring, visitor access and advanced accreditation. These need a real operational requirement, privacy/security decisions and tested fallback procedures.

Integrate rather than recreate initially: the learning engine, specialist payroll and accounting, payment execution, signing/identity services where required and current specialist applications. Native KSS workflow screens can still present status from those services.

Defer unless justified: public ticket sales, a full marketing suite, an independent tax/payroll engine, broad internal chat replacement, unsupervised AI deployment decisions and speculative predictive staff scoring.

The complete proposed navigation, access matrix, file/source structure, data entities, integration boundaries, phased roadmap and 24 cross-module test scenarios are included in Part C of the companion master specification.
