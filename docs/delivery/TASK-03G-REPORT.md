...
- Staging Auth redirects, Vercel environment variables, synthetic Site/onboarding data and application deployment remain pending. No unrelated Supabase project was used.
...
- Required before deployment: Vercel Preview-only environment configuration and readback; staging Auth URL; committed source; protection readback. Required after deployment: unauthorised protection denial; protected production build; actual URL role, file, desktop and mobile checks. Do not run destructive regression fixtures against the clean staging data.
