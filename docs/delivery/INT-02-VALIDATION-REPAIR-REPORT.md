...
| `tests/training-certificates-20e.test.mjs` | Prove the same Staff's credential claims/revisions/decisions and accepted SIA deployment candidate check remain unchanged through certificate issue, revocation, reissue and Completion void. |
...
- Added a bounded same-Person Certificate ≠ deployment eligibility assertion after that run. It creates one synthetic Event/SIA requirement and reads the existing `deployment_candidates` check for the same Staff before certificate issue and after issue, revocation, reissue and Completion void. The exact check remained unchanged at each point. The targeted 20E test passed **1/1** in 26.2 seconds. This assertion has **not** been run in a full serial suite yet; Candidate 2 remains the next full-suite target.
...
| Training Course → Assignment → Assessment → Completion → Certificate/PDF → revoke/reissue → void | **PASS** | 20B–E passed serially; same-Person Certificate ≠ Credential verification and Certificate ≠ deployment eligibility passed targeted. The latter awaits Candidate 2 full-suite inclusion. |
...
Operational Document and Certificate direct Storage denial remained in the passing 19A and 20E tests. Same-Person Document acknowledgement ≠ Training completion passed targeted. Allocation acceptance ≠ Attendance ≠ Worked Time and CRM WON ≠ Mobilisation ≠ Event delivery remain independently asserted by their passing domain tests; a combined same-record proof was not added. Training Certificate ≠ deployment eligibility is **PASS targeted** on the same Staff and accepted SIA candidate check; no new eligibility rule or deployment allocation was created for that assertion.

## Handoff verdict

The test-only patch resolves or stabilises all ten originally classified Candidate 1 failures in the full run. One new, non-reproducing TASK-10B statement timeout leaves the full-suite result **FAIL (76/77)**. No confirmed product-source defect emerged. David authorised INT-01 to review this patch and assemble Candidate 2 from frozen Candidate 1, with only test/harness changes. Do not merge this branch or declare Candidate 2 from this report alone. Candidate 2 needs its own static, targeted and single full serial validation against its exact frozen SHA; an isolated unchanged 10B rerun is permitted if the sole full-suite failure is again a statement timeout.
