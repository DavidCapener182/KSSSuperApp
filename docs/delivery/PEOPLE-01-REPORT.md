...
The first Development read returned HTTP **503** for the real authorised Office queue and its four preview reads. A read-only replay of five simultaneous guarded RPC calls reproduced PostgreSQL `57014` statement timeouts for all five. The same calls succeeded sequentially. The UI now serializes its queue reads, reuses the initial My cases result for that preview, defers team administration reads until needed, and retries one transient 503. In the subsequent authenticated local production preview, the full queue and all three additional previews returned HTTP **200**; the board showed source counts 205 active, 7 Needs Office, 198 Waiting for Staff, and 0 Blocked. This is a local Development readback, not staging or production acceptance.

[Actual authorised Development board readback](../product-review/people-01-evidence/kss-people-01-live-board-readback.png) shows the recovered queue. The four board lanes reported 7 Office, 198 Staff, 0 blocked and 184 My cases, with nine preview cards total.

The varied design review still uses **static synthetic browser responses** to the existing client API, with no database write, because the Development cases are near-identical. The case page's existing server access guard was left active and opened an already-authorised synthetic Development case route; the case API response shown in these screenshots is static synthetic review data. Those captures prove rendering and interaction layout only.
...
Focused ESLint, `npm run build` (Next 16.3.6 Webpack compile, TypeScript and static page generation) and `git diff --check` passed. Browser checks used installed Chrome against a local production preview at `127.0.0.1:4175`. The shared `127.0.0.1:3312` preview needs the follow-up UI loading fix integrated and restarted to show this last readback. Earlier real-data synthetic browser checks on the previous presentation had shown Staff/Office case access and direct 404 denial for unrelated Office and Operations plus protected document API/file; this revision did not repeat those denial checks.

## Remaining source gaps

The queue has a last-activity **timestamp** but no attributable event description. The case has no true activity/history feed, no native case-specific Training projection, and no authoritative stage or ready-to-close state. A future contract must specify scoped reads and source-owned status before those can be shown. The UI-only serialization resolves the observed local timeout burst, but the queue RPC still takes several seconds per view in synthetic Development; a source performance review would be needed before wider rollout. No compliance, pay, deployment or eligibility decision follows from these visuals.
