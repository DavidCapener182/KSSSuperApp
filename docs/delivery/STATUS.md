...
## Current boundary
Development remains on dedicated Supabase project `dnfhkmmnlbiabqypclqg`. Separate `KSS Enterprise - Staging` project `kwpgjbxepxuhwxxydaca` contains source-controlled migrations and four synthetic Auth/Person/role records, but no Site or onboarding case yet. David approved a separate Vercel `kss-enterprise-staging` project because Vercel classifies every new project's first successful build as Production. That staging-only project has Vercel Authentication on **All Deployments**, five staging Production-scope variables, Next.js preset and a GitHub connection. Its first build failed from an overbroad `.vercelignore`; commit `7796b93` fixed that and was pushed to `staging`. The corrected CLI upload failed before creating a deployment, so there is still no working staging application URL. The existing `kss-super-app` project also has no successful deployment. Development Staff A's Personal Details and SIA were resubmitted/reverified through normal authorised application routes, preserving prior history; that V2 case displays 5 of 6. Induction remains `NOT_CONNECTED` and the case remains `IN_PROGRESS`. No live KSS data, Entra, SharePoint, LMS, SIA register or working production application was added.

## Next recommended action
Complete a Git-backed build in the protected staging-only Vercel project, then configure staging Auth URLs and synthetic fixtures and run actual deployment checks in `TASK-03G-REPORT.md`. The LMS provider/interface is still unknown. Onboarding does not establish identity, legal RTW/SIA compliance or operational eligibility.
