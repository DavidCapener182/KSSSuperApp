...
## Further Operational Contacts history and access pass — for review

This later pass is separate from the accepted `b3d9b23` follow-up and is not yet accepted. It replaces the restricted history JSON dump with attributed event and published-version lists using only the already authorised history response. The history read remains reason-gated and audited by the existing server contract. Route revocation now waits for a current managed readback showing the exact route, revoked state and increased revision before confirmation. Grant issue and revocation wait for exact grant-ID readback from the existing grants view. An expiry action is reported as sent until the user reads its audited event; the existing managed projection has no expiry-event field, so the UI does not claim confirmation from that projection.
