...
test('08A stable Site shift demand, strict capacity and shared Event clash', {timeout:180000}, async()=>{
 ...
 const day=monday(plus(new Date().toISOString().slice(0,10),35));const next=plus(day,1);const stamp=Date.now();
 ...
 assert.ok(line);
 const detail=await rpc(operations,'site_service_detail',{p_site:site.data.id,p_service:service,p_from:day,p_until:plus(day,7)});
 assert.equal(detail.demands.length,7);const demand=detail.demands[0];assert.equal(demand.required_quantity,1);
 assert.equal((await rpc(office,'site_shift_generate',{p_service:service,p_from:day,p_until:plus(day,7)})),0);
 const allocation=await rpc(operations,'site_shift_allocate',{p_service:service,p_demand:demand.id,p_person:ids.staffA,
  p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic review acknowledged'});
 assert.ok(allocation);
 assert.ok((await operations.rpc('site_shift_allocate',{p_service:service,p_demand:demand.id,p_person:ids.staffB,
 ...
 assert.ok((await staffB.rpc('site_shift_respond',{p_allocation:allocation,p_expected_revision:2,p_response:'DECLINED',
  p_reason_code:'CANNOT_ATTEND'})).error);
 const preview=await rpc(staffA,'availability_preview',{p_starts:at(day,'07:00'),p_ends:at(day,'08:00')});
 assert.ok(preview.deployments.some((item)=>item.id===allocation&&item.source==='SITE_SHIFT'));
 const beforeConflict=await rpc(staffA,'my_availability',{});
 assert.ok((await staffA.rpc('availability_save',{p_state:'UNAVAILABLE',p_starts:at(day,'08:00'),
 ...
 await rpc(staffA,'availability_save',{p_state:'UNAVAILABLE',p_starts:at(day,'08:00'),
  p_ends:at(day,'09:00'),p_note:'Private synthetic test note',p_expected_revision:beforeConflict.revision,
 ...
 assert.ok((await office.rpc('deployment_allocate',{p_event:event,p_requirement:eventLine,p_person:ids.staffA,
  p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic review acknowledged'})).error);
 const eventAllocation=await rpc(office,'deployment_allocate',{p_event:event,p_requirement:eventLine,p_person:ids.staffB,
  p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic review acknowledged'});
 assert.ok((await office.rpc('site_shift_allocate',{p_service:service,p_demand:demand.id,p_person:ids.staffB,
  p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic review acknowledged'})).error);
 ...
 assert.ok((await staffA.rpc('site_shift_allocation_detail',{p_service:service,p_demand:demand.id})).error);
 ...
 assert.ok((await staffA.rpc('site_service_detail',{p_site:site.data.id,p_service:service,p_from:day,p_until:plus(day,7)})).error);
 ...
 const own=await rpc(staffA,'my_schedule_08a',{p_week:weekStart});
 ...
 const deployments=await rpc(staffA,'my_deployments_08a',{});
 assert.ok(deployments.items.some((item)=>item.source==='SITE_SHIFT'&&item.id===allocation));
 assert.equal((await rpc(staffA,'my_deployments_08a',{p_focus:eventAllocation})).total,0);
 const person=await rpc(operations,'workforce_person_week_08a',{p_person:ids.staffA,p_week:weekStart});
 ...
 const boundaryAllocation=await rpc(operations,'site_shift_allocate',{p_service:service,p_demand:boundaryDemand,p_person:ids.staffA,
 ...
 const [eventRace,siteRace]=await Promise.all([
  office.rpc('deployment_allocate',{p_event:raceEvent,p_requirement:raceLine,p_person:ids.staffB,
   p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic cross-source race'}),
  operations.rpc('site_shift_allocate',{p_service:service,p_demand:gap.id,p_person:ids.staffB,
   p_expected_revision:1,p_acknowledge_warnings:true,p_reason:'Synthetic cross-source race'})]);
 ...
 const availabilityBefore=await rpc(staffA,'my_availability',{});
 await rpc(staffA,'availability_save',{p_state:'UNAVAILABLE',p_starts:at(unavailableDay,'08:00'),
 ...
 assert.ok((await office.rpc('site_shift_allocate',{p_service:service,p_demand:unavailableDemand.id,p_person:ids.staffA,
 ...
 assert.equal(afterPause.demands.find((item)=>item.service_date===plus(day,4)).state,'CANCELLED');
