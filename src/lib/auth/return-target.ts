const ALLOWED_PATHS = new Set(["/app", "/work", "/sites", "/profile", "/documents"]);
...
export function safeReturnTarget(value: unknown): string | null {
  ...
  try {
    ...
    if (url.origin !== "https://kss.invalid" ||
      !(ALLOWED_PATHS.has(url.pathname) || /^\/(documents|work)\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(url.pathname))) return null;
