...
export function safeReturnTarget(value: unknown): string | null {
  ...
  try {
    ...
    if (url.origin !== "https://kss.invalid" ||
      !(ALLOWED_PATHS.has(url.pathname) || /^\/(documents|work|onboarding|people|events|mobilisations|incidents|site-book)\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(url.pathname) || /^\/events\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\/(attendance|work-time)$/i.test(url.pathname))) return null;
