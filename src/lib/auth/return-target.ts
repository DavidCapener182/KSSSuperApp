const ALLOWED_PATHS = new Set(["/app", "/work", "/sites", "/events", "/my-deployments", "/my-availability", "/profile", "/documents", "/onboarding", "/people"]);
...
export function safeReturnTarget(value: unknown): string | null {
  ...
  try {
    ...
    if (url.origin !== "https://kss.invalid" ||
      !(ALLOWED_PATHS.has(url.pathname) || /^\/(documents|work|onboarding|people)\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(url.pathname))) return null;
