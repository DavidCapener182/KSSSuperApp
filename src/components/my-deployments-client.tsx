...
export function MyDeploymentsClient({focus,focusSource}:{focus?:string;focusSource?:"EVENT"|"SITE_SHIFT"}) {
  ...
  const card = (item: Deployment) => <article className="crm-panel deployment-self-card" key={item.id}>
    ...
    {item.source === "EVENT" && item.status === "ACCEPTED" && <div className="deployment-actions"><Button asChild variant="outline"><Link href={`/my-attendance?allocationId=${encodeURIComponent(item.id)}`}>Open attendance</Link></Button></div>}
  </article>;
