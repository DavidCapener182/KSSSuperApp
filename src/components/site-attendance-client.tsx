...
export function SiteAttendanceClient({siteId,serviceId,demandId}:{siteId:string;serviceId:string;demandId?:string}){
  ...
  const load=useCallback(async(page=0):Promise<Row[]|null>=>{setLoading(true);try{const q=new URLSearchParams({offset:String(page)});if(demandId)q.set("demandId",demandId);const response=await fetch(`${base}?${q}`,{cache:"no-store"});const body=await response.json();if(!response.ok)throw new Error(body.error??"Attendance unavailable");const next:Row[]=body.attendance?.items??[];setRows(next);setTotal(body.attendance?.total??0);setOffset(page);setError("");return next;}catch(caught){setError(caught instanceof Error?caught.message:"Attendance unavailable");return null;}finally{setLoading(false);}},[base,demandId]);
  ...
  async function submit(row:Row,action:string,targetEventId?:string,actualAt?:string){
    ...
    try{const response=await fetch(base,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const body=await response.json().catch(()=>({}));if(!response.ok)throw new Error(body.error??"Attendance was not recorded. Refresh and retry safely.");localStorage.removeItem(key);const current=await load(offset);if(current?.find(item=>item.allocation_id===row.allocation_id&&item.attendance.revision>row.attendance.revision))setNotice("Attendance fact confirmed in the current source record.");else setError("The action was accepted, but the new attendance revision could not be confirmed. Refresh before acting again.");}
