...
export function ManagementReportsClient() {
  ...
  return <main className={`enterprise-main ${styles.page}`}>
    ...
    <details className={styles.filters}><summary>Filters</summary><div className={styles.filterGrid}>
    ...
    {report && <>
      ...
      {report.status ? <section className={styles.warning} role="status"><h2>{report.status}</h2><p>{report.reason}</p></section> : <>
        <section aria-labelledby="period-heading"><h2 id="period-heading">Operational demand during period</h2><div className={styles.cards}>{metrics.map(([label, value, code]) => <article className={styles.card} key={code}><h3>{label}</h3><strong>{value?.toLocaleString("en-GB") ?? "Unavailable"}</strong><small>Definition {code} v{report.definitions[code]}</small></article>)}</div>
        ...
        <section aria-labelledby="breakdown-heading"><h2 id="breakdown-heading">Breakdown by Client, Site, Service/Event and date</h2><div className={styles.rows}>{report.breakdowns?.map((row, index) => <article className={styles.row} key={`${row.source}-${row.parent_id}-${row.service_date}-${index}`}><div><strong>{row.client_name} · {row.site_name}</strong><p>{row.source === "EVENT" ? "Event" : "Site Shift"} · {row.parent_name} · {row.service_date}</p></div><p>Required {row.required} · Allocated {row.allocated} · Accepted {row.accepted} · Remaining {row.remaining}</p></article>)}</div></section>
        <section aria-labelledby="lines-heading"><h2 id="lines-heading">Contributing source lines</h2><p>{report.total_lines ?? 0} lines; page {Math.floor(offset / 30) + 1}. Totals include all authorised lines for these filters.</p><div className={styles.rows}>{report.lines?.map((line) => <article className={styles.row} key={`${line.source}-${line.source_id}`}><div><strong>{line.source === "EVENT" ? "Event requirement" : "Site Shift demand"} · {line.service_date}</strong><p>{line.client_name} · {line.site_name} · {line.parent_name}</p><p>Source UUID: <code>{line.source_id}</code> · Parent UUID: <code>{line.parent_id}</code> · {line.source_state} · Definition v{line.definition_version}</p></div><div><p>Required {line.required} · Allocated {line.allocated} · Accepted {line.accepted} · Remaining {line.remaining}</p><Link href={destination(line)}>Open source record</Link></div></article>)}</div>
