...
export function MyAttendanceClient(){
 const [items,setItems]=useState<Item[]>([]);const [loading,setLoading]=useState(true);const [busy,setBusy]=useState("");const [error,setError]=useState("");const [notice,setNotice]=useState("");
 const load=useCallback(async()=>{setLoading(true);try{const response=await fetch("/api/attendance/me",{cache:"no-store"});if(!response.ok)throw new Error("Attendance is unavailable. Retry to refresh.");const data=await response.json();setItems(data.attendance?.items??[]);setError("");}catch(e){setError(e instanceof Error?e.message:"Attendance unavailable");}finally{setLoading(false);}},[]);
 ...
 return <section className={styles.wrap}>
  ...
  {loading?<p role="status">Loading your attendance…</p>:items.length===0?<p className={styles.empty}>No Event allocations are available.</p>:items.map(item=>{
