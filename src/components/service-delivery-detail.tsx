...
const recordSections = ["management-context", "review-periods", "source-facts", "meetings", "actions", "management", "history"] as const;
...
export function ServiceDeliveryDetail({ id, superAdmin }: { id: string; superAdmin: boolean }) {
  ...
  const [activeSection,setActiveSection] = useState<RecordSection>("management-context");
  const sectionNav = useRef<HTMLElement | null>(null);
  useEffect(() => { const sync = () => { const hash = window.location.hash.slice(1); setActiveSection(recordSections.includes(hash as RecordSection) ? hash as RecordSection : (hash === "commitments" || hash === "changes") ? "management" : "management-context"); };
  ...
  return <main className={`enterprise-main ${styles.page}`}>
    ...
    <nav ref={sectionNav} className="service-record-sections" aria-label="Service Delivery sections"><a href="#management-context" aria-current={activeSection==="management-context"?"location":undefined} onClick={event=>selectSection(event,"management-context")}>Management context</a><a href="#review-periods" aria-current={activeSection==="review-periods"?"location":undefined} onClick={event=>selectSection(event,"review-periods")}>Review periods</a><a href="#source-facts" aria-current={activeSection==="source-facts"?"location":undefined} onClick={event=>selectSection(event,"source-facts")}>Source facts</a><a href="#meetings" aria-current={activeSection==="meetings"?"location":undefined} onClick={event=>selectSection(event,"meetings")}>Meetings</a><a href="#actions" aria-current={activeSection==="actions"?"location":undefined} onClick={event=>selectSection(event,"actions")}>Actions &amp; blockers</a><a href="#management" aria-current={activeSection==="management"?"location":undefined} onClick={event=>selectSection(event,"management")}>Commitments &amp; changes</a><a href="#history" aria-current={activeSection==="history"?"location":undefined} onClick={event=>selectSection(event,"history")}>History</a></nav>
    ...
    <div hidden={activeSection!=="management-context"}>
    ...
    <section id="review-periods" hidden={activeSection!=="review-periods"}><h2>Review periods</h2><p>Custom Europe/London dates. Periods never close automatically.</p>
    ...
    <div hidden={activeSection!=="source-facts"}><ServiceDeliverySourceCards deliveryId={d.id} periods={d.periods} /></div>
    <section id="meetings" hidden={activeSection!=="meetings"}><h2>Meetings</h2><p>A scheduled meeting is not a held meeting. Held records occurrence only.</p>
    ...
    <section id="actions" hidden={activeSection!=="actions"}><h2>Actions &amp; blockers</h2>
    ...
    <div id="management" hidden={activeSection!=="management"}><ServiceDeliveryManagement deliveryId={d.id} deliveryRevision={d.revision} terminal={terminal} /></div>
    <section id="history" hidden={activeSection!=="history"}><h2>History <small>{historyTotal}</small></h2><p>From the first event, 25 at a time.</p>
