...
const recordSections = ["management-context", "review-periods", "source-facts", "meetings", "actions", "history"] as const;
...
export function ServiceDeliveryDetail({ id, superAdmin }: { id: string; superAdmin: boolean }) {
  ...
  useEffect(() => { const sync = () => { const hash = window.location.hash.slice(1); setActiveSection(recordSections.includes(hash as RecordSection) ? hash as RecordSection : "management-context"); };
