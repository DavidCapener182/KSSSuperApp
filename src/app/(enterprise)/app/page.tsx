...
export default async function AppHome() {
  ...
  return <main className="enterprise-main">
    ...
    <div className="enterprise-card-grid">
      {links.map((link) => <Link className="enterprise-card" href={link.href} key={link.href}><strong>{link.label}</strong><span>{link.href === "/people" ? "Find staff and open a permission-scoped staff record." : link.href === "/work" ? "Open your assigned document review work." : link.href === "/sites" ? "Open the Sites you are authorised to use." : link.href === "/documents" ? "View your authorised synthetic document requests." : link.href === "/onboarding" ? "See your authorised synthetic starter checklist and next actions." : "View your current Enterprise identity and roles."}</span></Link>)}
    </div>
