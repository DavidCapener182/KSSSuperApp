...
export default async function AppHome() {
  ...
  return <main className="enterprise-main">
    ...
    <div className="enterprise-card-grid">
      {links.map((link) => <Link className="enterprise-card" href={link.href} key={link.href}><strong>{link.label}</strong><span>{link.href === "/sites" ? "Open the Sites you are authorised to use." : "View your current Enterprise identity and roles."}</span></Link>)}
    </div>
