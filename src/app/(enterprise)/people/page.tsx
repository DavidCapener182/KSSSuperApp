...
export default async function PeoplePage({ searchParams }: { searchParams: Query }) {
  ...
  return <main className="enterprise-main people-page">
    <p className="eyebrow">KSS directory · synthetic development data</p>
    <h1>People</h1>
    <p className="enterprise-intro">Find KSS staff and open a safe staff record. Private details remain separately permission controlled.</p>
    ...
    <div className="people-result-heading"><h2>Directory</h2><p>{result.total} {result.total === 1 ? "person" : "people"} in this view</p></div>
    {result.items.length === 0 ? <section className="people-empty"><h3>No matching People</h3><p>Try a different name or clear the filters.</p><Link href="/people">Clear filters</Link></section> : <>
      ...
      <ul className="people-mobile-list">{result.items.map((person) => <li key={person.id} className="people-mobile-card"><div><strong>{person.displayName}</strong><span>{person.roles.length ? person.roles.map(label).join(" · ") : "No active role"}</span></div><p>{position(person)}</p><p>{person.sites.length ? person.sites.join(" · ") : "No Site context shown"}</p><Link href={personHref(person)}>View staff record</Link></li>)}</ul>
    </>}
