...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await client.rpc("cancel_onboarding_case", { requested_case: c.id });
  return error ? privateJson({ error: "Onboarding cancellation denied" }, 403) : privateJson({ id: c.id });
}
