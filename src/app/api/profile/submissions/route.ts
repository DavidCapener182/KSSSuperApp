...
export async function POST(request: Request) {
  ...
  if (!isUuid(body?.caseId) || !isUuid(body?.requestKey)) return privateJson({ error: "Invalid submission" }, 400);
  ...
  if (!c || c.personId !== principal.personId || c.state !== "IN_PROGRESS" || c.templateVersion < 2)
    return forbidden();
  const { data, error } = await client.rpc("submit_onboarding_profile", {
    requested_case: c.id, supplied_request_key: body.requestKey,
  });
  return error || !data ? privateJson({ error: "Personal Details incomplete or submission denied" }, 400)
